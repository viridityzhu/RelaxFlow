...
def render_frames_rgba(
    ...
    **kwargs,
    ...
    options_black = dict(options)
    options_white = dict(options)
    options_black["bg_color"] = (0, 0, 0)
    options_white["bg_color"] = (1, 1, 1)
    res_black = render_frames(
        sample,
        extrinsics,
        intrinsics,
        options_black,
    ...
    rgba_frames = []
    for rgb_black, rgb_white in zip(res_black.get("color", []), res_white.get("color", [])):
        rgb_black_f = rgb_black.astype(np.float32) / 255.0
        rgb_white_f = rgb_white.astype(np.float32) / 255.0
        alpha = 1.0 - (rgb_white_f - rgb_black_f).mean(axis=-1, keepdims=True)
        alpha = np.clip(alpha, 0.0, 1.0)
        rgba = np.concatenate([rgb_black_f, alpha], axis=-1)
        rgba_frames.append((rgba * 255).astype(np.uint8))
    return {"color": rgba_frames}
