...
class EmbedderFuser(torch.nn.Module):
    ...
    def _filter_tokens_by_name(
        ...
        """
        Reduce the fused condition to a subset of modalities.
        """
        assert all(name in kwarg_names for name in keep_names), f"Requested modalities {keep_names} not found in kwarg_names: {kwarg_names}"

        result_tokens = []
        
        for kwarg_name, token_tensor in zip(kwarg_names, tokens):
            # Create mask: 0 for forced drop, 1 otherwise
            mask = 0.0 if (kwarg_name not in keep_names) else 1.0
            result_tokens.append(token_tensor * mask)
        
        return result_tokens

    def forward(self, *args, **kwargs):
        only_cropped_img = kwargs.pop("only_cropped_img", False)
        only_cropped_img_and_mask = kwargs.pop("only_cropped_img_and_mask", False)
        ...
        if only_cropped_img:
            # Keep only the cropped image token when requested.
            # tokens = self._filter_tokens_by_name(kwarg_names, tokens, {"image", "mask"})
            # all condition keys: ['image', 'rgb_image', 'mask', 'rgb_image_mask', 'pointmap', 'rgb_pointmap']
            keep_names = {"image"}
            tokens = self._filter_tokens_by_name(kwarg_names, tokens, keep_names)
            print(f'kept condition keys: {keep_names}')
        elif only_cropped_img_and_mask:
            # Keep only the cropped image token and its occluded mask when requested.
            # tokens = self._filter_tokens_by_name(kwarg_names, tokens, {"image", "mask"})
            # all condition keys: ['image', 'rgb_image', 'mask', 'rgb_image_mask', 'pointmap', 'rgb_pointmap']
            keep_names = {"image", "mask"}
            tokens = self._filter_tokens_by_name(kwarg_names, tokens, keep_names)
            print(f'kept condition keys: {keep_names}')
