...
class FlowMatching(Base):
    ...
    def _sample_with_generator(self, x_shape, target_device):
        """
        Sample Gaussian noise using the module's dedicated generator so we can
        deterministically control randomness per-model (important for video
        sequences where we want fixed noise across frames).
        """
        if target_device is None:
            target_device = torch.device("cpu")
        target_device = torch.device(target_device)

        generator = getattr(self, "_random_generator", None)
        if generator is None:
            return torch.randn(x_shape, device=target_device)
        ...
        if generator.device != target_device:
            noise = noise.to(target_device)
        return noise

    def _generate_noise_tensor(self, x_shape, x_device):
        return self._sample_with_generator(x_shape, x_device)
    ...
    def _generate_x0_tensor(self, x1: torch.Tensor):
        return self._sample_with_generator(x1.shape, x1.device)
