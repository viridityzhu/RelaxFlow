...
def set_attention_backend():
    if torch.cuda.is_available():
        ...
        logger.info("GPU name is {}", gpu_name)
        if "A100" in gpu_name or "H100" in gpu_name or "H200" in gpu_name:
            # logger.info("Use flash_attn")
            os.environ["ATTN_BACKEND"] = "flash_attn"
            os.environ["SPARSE_ATTN_BACKEND"] = "flash_attn"
    else:
        logger.info("CUDA not available; defaulting attention backend to PyTorch.")
...
class InferencePipeline:
    def __init__(
        ...
        if self.device.type == "cuda" and torch.cuda.is_available():
            logger.info("Actually using GPU: {}", torch.cuda.current_device())
        else:
    ...
    def _resolve_seed_triplet(self, seed):
        """
        Normalize user supplied seed input into (global_seed, ss_seed, slat_seed).
        Allows ints, (shape, texture) tuples, or dicts.
        """
        if seed is None:
        ...
        if isinstance(seed, dict):
            base = seed.get("global")
            ss_seed = seed.get("shape", base)
            slat_seed = seed.get("texture", base)
            if base is None:
                base = ss_seed if ss_seed is not None else slat_seed
            return _as_int(base), _as_int(ss_seed), _as_int(slat_seed)

        if isinstance(seed, (tuple, list)):
            ss_seed = seed[0] if len(seed) > 0 else None
            slat_seed = seed[1] if len(seed) > 1 else None
            base = ss_seed if ss_seed is not None else slat_seed
            return _as_int(base), _as_int(ss_seed), _as_int(slat_seed)

        seed_int = int(seed)
        return seed_int, seed_int, seed_int

    def _make_generator_for_seed(self, seed_value):
        ...
        try:
            ...
            candidate_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        if torch.cuda.is_available() and candidate_device.type == "cuda":
            resolved_device = candidate_device

        generator = torch.Generator(device=resolved_device)
        generator.manual_seed(int(seed_value))
        return generator

    def set_model_seeds(self, ss_seed=None, slat_seed=None):
        """
        Attach deterministic torch.Generator instances to the flow models so that
        noise sampling can be reproduced independently from other random calls.
        """
        if ss_seed is not None and "ss_generator" in self.models:
            generator = self._make_generator_for_seed(ss_seed)
            if generator is not None:
                self.models["ss_generator"].random_generator = generator

        if slat_seed is not None and "slat_generator" in self.models:
            generator = self._make_generator_for_seed(slat_seed)
            if generator is not None:
                self.models["slat_generator"].random_generator = generator
    ...
    def merge_image_and_mask(
        ...
        if mask is not None:
            ...
            # check if mask is [0,1] and if so, convert to [0,255]
            if mask.max() <= 1.0:
                mask = mask * 255
    ...
    def get_condition_input(
        ...
        input_dict,
        input_mapping,
        extra_condition_kwargs: Optional[dict] = None,
    ):
        condition_args = self.map_input_keys(input_dict, input_mapping)
        condition_kwargs = {
            k: v for k, v in input_dict.items() if k not in input_mapping
        }
        if extra_condition_kwargs:
            overlap = set(condition_kwargs.keys()) & set(extra_condition_kwargs.keys())
            if overlap:
                raise ValueError(
                    f"Duplicate condition kwargs detected: {', '.join(sorted(overlap))}"
                )
            condition_kwargs = {**condition_kwargs, **extra_condition_kwargs}
